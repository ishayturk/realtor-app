...
# Status: Syntax Fixed | Protocol: Full File Delivery
# ==========================================
...
if st.session_state.step == "login":
    ...
    with c_right:
        st.markdown('<div style="text-align: right; font-weight: bold;">🏠 מתווך בקליק</div>', unsafe_allow_html=True)
    with c_mid:
        st.markdown(f'<div style="text-align: center; font-weight: 900;">{st.session_state.user}</div>', unsafe_allow_html=True)
    with c_left:
            ...
            if not st.session_state.quiz_active:
                if st.button("📝 שאלון תרגול"):
                    with st.spinner("מכין שאלה..."):
                        res = fetch_q_ai(st.session_state

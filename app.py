...
def apply_design():
    ...
    <style>
        ...
        .stButton button { width: 100% !important; height: 3.5em !important; border-radius: 12px !important; }
...
def fetch_quiz(model, topic):
    prompt = f"Create 10 MCQs in HEBREW about {topic}. Return ONLY JSON array: [{{'q':'','options':['','','',''],'correct':0,'explanation':''}}]"
...
def main():
    ...
    st.markdown('<div class="main-header"><h1>🏠 מתווך בקליק</h1><p>לימוד רציף ומקצועי</p></div>', unsafe_allow_html=True)
    ...
    # --- דף תפריט ראשי ---
        ...
        FULL_SYLLABUS = ["חוק המתווכים במקרקעין", "חוק המקרקעין", "חוק המכר (דירות)", "חוק החוזים", "חוק הגנת הצרכן", "חוק הגנת הדייר", "חוק התכנון והבנייה", "חוק מיסוי מקרקעין", "חוק העונשין", "חוק שמאי מקרקעין", "חוק הירושה", "חוק יחסי ממון", "חוק איסור הלבנת הון", "פקודת הנזיקין", "מושגי יסוד בכלכלה", "רשות מקרקעי ישראל"]
    ...
    # --- דף שיעור (כולל Streaming) ---
        ...
        if not st.session_state.lesson:
            full_text = ""
            response = model.generate_content(f"כתוב שיעור מפורט בעברית על {st.session_state.topic}.", stream=True)
        ...
        if st.button("סיימתי ללמוד, עבור לתרגול ✍️"):
            with st.spinner("מכין שאלות על בסיס השיעור..."):

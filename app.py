# ==========================================
# Project: מתווך בקליק | Version: 1199
# ==========================================
...
def stream_ai_lesson(p):
    """פונקציה המבצעת סטרימינג של השיעור ישירות למסך"""
    try:
        genai.configure(api_key=st.secrets["GEMINI_API_KEY"])
        m = genai.GenerativeModel('gemini-2.0-flash')
        full_p = p + " כתוב שיעור הכנה מעמיק, ארוך ומפורט מאוד למבחן המתווכים. פרט סעיפי חוק, מספרים, ימים, אחוזים ודוגמאות מעשיות. אל תכתוב כותרות."
...
if "step" not in st.session_state:
    st.session_state.update({"user": None, "step": "login", "q_count": 0, "quiz_active": False, "show_ans": False, "lesson_txt": ""})
...
if st.session_state.step == "login":
    ...
    for i, s in enumerate(subs):
        if cols[i].button(s, key=f"btn_{i}"):
            ...
            st.session_state.lesson_txt = "LOADING" # טריגר לסטרימינג
            st.rerun()
    ...
    if st.session_state.get("lesson_txt") == "LOADING":
        ...
        st.session_state.lesson_txt = content # שומר את התוכן אחרי הסטרימינג
    elif st.session_state.get("lesson_txt"):
        st.subheader(st.session_state.current_sub)
        st.markdown(st.session_state.lesson_txt)

    if st.session_state.quiz_active and st.session_state.get("q_data"):
        st.divider()
    ...
    st.write("---")
    foot_cols = st.columns([2, 1.5, 1.5, 4])
    ...
    st.caption(f"Version: 1199 | User: {st.session_state.user}")

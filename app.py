...
SYLLABUS = {
    ...
    "שונות": ["מקרקעי ישראל (רמ״י)", "הוצאה לפועל ומשכנתאות", "שמאי מקרקעין", "הגנת הדייר"]
}
...
# -------------------------
    ...
    if st.session_state.get("current_sub"):
        if st.session_state.get("lesson_txt") == "LOADING":
            st.session_state.lesson_txt = stream_ai_lesson(st.session_state.current_sub)
            st.rerun()
        elif st.session_state.get("lesson_txt") is None:
            st.error("⚠️ הייתה תקלה בטעינת השיעור. אנא נסה נושא אחר כרגע ומאוחר יותר חזור לנושא הזה.")

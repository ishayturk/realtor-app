# גרסה: 1025 | תאריך: 16/02/2026 | שעה: 00:20
# מזהה פרויקט: REALTOR_EXAM_SIM_PRO_V2
# מפתח: Gemini AI Collaboration
# סטטוס: Fixed Logic - Quiz (Full Load) | Exam (Background Load)
...
if S.step == "login":
    ...
    if not S.lt:
        ...
        if c1.button(f"✍️ שאלון תרגול"):
            with st.spinner("מכין את כל השאלות..."):
                # שאלון טוען הכל (10) בבת אחת כדי שלא ייתקע
    ...
    if S.mode == 'study_quiz' and S.qi in S.cq:
        st.info(f"הסבר: {it['reason']}") if S.qans.get(S.qi) == it['correct'] else st.error(f"טעות. הנכון: {it['correct']}. {it['reason']}")

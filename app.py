# גרסה: 1027 | תאריך: 16/02/2026 | שעה: 00:40
# מזהה פרויקט: REALTOR_EXAM_SIM_PRO_V2
# מפתח: Gemini AI Collaboration
# סטטוס: Simple & Working - No unnecessary reruns
...
if S.step == "login":
    ...
    if S.lt:
        if not full_text: # מציג את השיעור אם כבר קיים ב-session
            st.markdown(f"<div class='lesson-box'>{S.lt}</div>", unsafe_allow_html=True)
        
        if st.button(f"✍️ עבור לשאלון תרגול על {S.current_topic}"):

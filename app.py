# גרסה: 1003 | תאריך: 15/02/2026 | שעה: 20:40
...
# כותרת גרסה
st.markdown("<div style='text-align: left; color: gray; font-size: 10px;'>גרסה: 1003 | 15/02/2026 | 20:40</div>", unsafe_allow_html=True)
...
def get_questions(topic, count):
    try:
        ...
        p = f"צור {count} שאלות למבחן המתווכים בנושא {topic}. החזר JSON נקי: [{{'q':'','options':['א','ב','ג','ד'],'correct':'טקסט מדויק','reason':''}}]"
...
if S.step == "login":
    ...
    S.qa = False # איפוס שאלון כשחוזרים לתפריט
    c1, c2 = st.columns(2)
        ...
        if not S.qa:
            ...
            if c1.button(f"✍️ שאלון: {S.current_topic}"):
                with st.spinner("טוען שאלות..."):
                    ...
                    if d: S.qq, S.qa, S.qi, S.cq, S.total_q = d, True, 0, set(), 10; st.rerun()
            if c2.button("🏁 חזרה לתפריט"): S.step, S.lt = "menu", ""; st.rerun()
        else: S.step = "quiz_mode"; st.rerun()

elif S.step == "exam_lobby":
    st.write("### סימולציית מבחן מלאה (25 שאלות)")
    st.write("המבחן כולל שאלות מכל נושאי הלימוד. 5 שאלות ראשונות ייטענו מיד.")
    if st.button("🚀 התחל מבחן"):
        with st.spinner("מייצר שאלות..."):
            d = get_questions("דיני מקרקעין, חוק המתווכים וחוזים", 5)
            if d: S.qq, S.qa, S.qi, S.cq, S.total_q, S.step = d, True, 0, set(), 25, "quiz_mode"; st.rerun()
    if st.button("🏠 חזרה"): S.step = "menu"; st.rerun()

elif S.step == "quiz_mode":
    it = S.qq[S.qi]
    st.write(f"### שאלה {S.qi+1} מתוך {S.total_q}")
    ans = st.radio(it['q'], it['options'], key=f"q_{S.qi}", index=None)
    
    # הצגת הסבר במידה וענה
    if S.qi in S.cq:
        corr = str(it['correct']).strip()
        is_ok = str(S.qans.get(S.qi)).strip() == corr
        if is_ok:
    ...
    if ans and S.qi not in S.cq:
        if b_cols[0].button("🔍 בדוק"): S.qans[S.qi] = ans; S.cq.add(S.qi); st.rerun()
    
    if S.qi in S.cq:
        if S.qi < S.total_q - 1:
            if b_cols[1].button("➡️ השאלה הבאה"):
                if S.qi == len(S.qq)-1: # טעינת עוד שאלות בסימולציה
                    with st.spinner("טוען עוד שאלות..."):
                        more = get_questions("דיני מקרקעין", 5)
                        if more: S.qq.extend(more)
                S.qi += 1; st.rerun()
        else:
            if b_cols[1].button("🏁 סיים מבחן"): S.step, S.lt, S.qa = "menu", "", False; st.rerun()
            
    if b_cols[2].button("🏠 תפריט"): S.step, S.lt, S.qa = "menu", "", False; st.rerun()

...
def generate_lesson_content(topic):
    prompt = f"צור שיעור על {topic} למתווכים. חלק ל-3 תתי נושאים. לכל חלק הוסף שאלה אמריקאית. החזר JSON בלבד עם מפתחות: sub_topics (title, content, question (q, options, correct))"
    res = ask_ai(prompt)
    try:
